...
// 得知管道
// $_POST[custom_U179006] = $_POST[custom_U179006];
...
if ($sql->num_rows == 0) {
    ...
    // 先 comment 掉不使用 random 名稱
    // shuffle($list_random_name);

    // 將手機存成密碼
    $password = eregPhone($_POST['custom_U807']);
    $password_md5 = md5($password);
    $str = "Insert Into `user` (`active`, `email`, `password`, `name`, `phone_country`, `phone_number`, `age`, `gender`, `how_to_know`, `register_ip`, `register_time`) Values ('1', '$_POST[Email]', '$password_md5', '（匿名使用者）', '$_POST[custom_U8475]', '$_POST[custom_U807]', '$_POST[custom_U5503]', '$_POST[custom_U2291]','$_POST[custom_U179006]', '$_SERVER[REMOTE_ADDR]', '$now')";
    $mysql->query($str);
