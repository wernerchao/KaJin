...
function list_counselor($gender = -1)
{
    ...
    if ($gender == 0 || $gender == 1) {
        ...
        $sql_string = 'Select * From `counselor` WHERE `active`=1 Order By `id`';
    }
...
function notification($type, $email, $var = array())
{
    ...
    if ($type == 'new_counselor_to_admin') {
        $message = "老師姓名：$var[name]\n<br>性別：$var[gender]\n<br>時間: ".date('Y-m-d H:i:s', $system['time_now'])."\n<br>E-mail: $var[email]\n<br>聯絡電話: $var[tel]";
        $title = '新老師註冊通知';
    }
