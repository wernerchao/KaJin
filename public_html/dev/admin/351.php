...
if($_POST['weekstart']) $_GET['weekstart'] = $_POST['weekstart'];
...
if($_POST['submit'])
{
	...
	while($row = $sql->fetch_assoc()) $appointment_data[$row['date']][$row['time']] = $row;
	// 開始增刪時段
	$date = $start;
	for($i=0; $i<=6; $i++)
	{
		...
		if($date >= get_date())
		{
			...
			{
				// 當天的已過小時不處理
				if($date == get_date() && $j < date("H", $system['time_now'])) continue;
				// 處理 $date 日的 $j 點
				// 1. 如果這是已有人預約則不可動
				if($_POST[$date."-".$j] == -1 || $appointment_data[$date][$j]['user_id']) continue;
				// 2. 如果要改成可預約但現在沒有則新增
...
for($i=0; $i<=6; $i++)
{
	...
	for($j=0; $j<=23; $j++)
	{
		if($date < get_date(array("range" => -7))) $val = -2;
		else if($date == get_date(array("range" => -7)) && $j < date("H", $system['time_now'])) $val = -2;
		else if($appointment_data[$date][$j]['id']) $val = 1;
		else $val = 0;
		echo $val.(($j==23)?"":",");
	}
	echo "];";
...
for($j=0; $j<=23; $j++) echo "<th>".$j."</th>";
echo "<th>從上週複製</th>";
echo "</tr>";
for($i=0; $i<=6; $i++)
{
	echo "<tr>";
	if($date == get_date()) echo "<td bgcolor=\"#ffaaaa\">".$date."</td><td bgcolor=\"#ffaaaa\">".$list_week[get_week($date)]."</td>";
	else echo "<th>".$date."</th><th>".$list_week[get_week($date)]."</th>";
	for($j=0; $j<=23; $j++)
	{
		...
		else if($date < get_date() || ($date == get_date() && $j < date("H", $system['time_now'])))
		{
		...
		else if($appointment_data[$date][$j]['id'])
		{
