...
(function(){
    ...
    $(document).ready(function(){
        ...
        setTimeout(function(){
            ...
            $.getJSON('data.json',function(data){
                $.each(data.consultant,function(index,value){
                    $('div#colelem-choose .container').append(//把專長經歷都放上去
                                    '<div class="consultantBlock" data-id="'+ index +'">'+
                                        '<div class="elementchoose">選擇</div>'+
                                        '<div class="elementphoto" style="background-image:url('+ data.imageLocation + value.photo +')"></div>'+
                                        ...
                                        '</div>'+
                                        '<div class="elementname">'+ value.wholeName +'</div>'+
                                        '<div class="elementline"></div>'+
                                        '<div class="elementdescribe">'+
                                            '<p>'+ value.shortWords  +'</p>'+
                                        '</div>'+
                                    '</div>'
                                    );
                    $('#page').append(//個人檔案iframe
                                '<div class="consultantprofile" data-id="'+index+'">'+
                                    ...
                                    '<iframe src="'+ data.profileLocation + value.profile +'" frameborder="0" class="profile"></iframe>'+
                                '</div>'
                            );
                });
                //為所有新增的元件綁上listener
        ...
        $('#step1 #colelem-finish-choose .next-step').on('click',function(){//確認
            ...
            $.getJSON('data.json',function(data){
                availableTeacher.forEach(function(value,index){
                                            ...
                                            '<div class="content">'+
                                                '<div class="col1 col">'+
                                                    ...
                                                    '<div class="name-block">'+
                                                        '<div class="name">'+data.consultant[value].name+'</div>'+
                    ...
                    $('#step2 .grey-container1 .content .photo ').eq(index).css('background-image','url('+ data.imageLocation + data.consultant[value].photo+')');
                    
                    //替老師資料建表
                    availableTeacherProperty.push({
                        name:data.consultant[value].name,
                        photo:'url('+ data.imageLocation + data.consultant[value].photo+')'
                    });
                });
        ...
        $('#step2 .next-step').on('click',function(){//下一步
            if($('.select-date').html()==='')
                ...
                //$('#step4 .grey-container1 .name-block .name').html(getSelectTeacherName());
                $('#step3 .grey-container1 .photo').css('background-image',getSelectTeacherPhoto());
                //$('#step4 .grey-container1 .photo').css('background-image',getSelectTeacherPhoto());
        ...
        $('#step3 #book-confirm .next-step').on('click',function(){
            ...
            var handler = StripeCheckout.configure({
                ...
                //key: 'pk_live_DsZ9kTZ8onNEmJXR690i6rwR',
        ...
        $('#step-need-help #colelem2 .next-step').on('click',function(){
            $('#step-need-help .step-div button').attr('disabled',true);
            $.ajax({
        ...
        $('#step-need-help .hint-greybox .next-step').on('click',function(){
    ...
    function setSchedule(schedule,shift){//設定時間表
        ...
        $.each(tmpSchedule,function(index){ tmpSchedule[index]=[]; });
            ...
            function appendConsultant(id){
                if(id===0)
                    ...
                    $('.choose-card-div').append(
                            ...
                            '<div class="elementphoto" style="background-image:url('+jsondata.imageLocation + jsondata.consultant[id].photo +'"></div>'+
                            '<div class="elementname">'+jsondata.consultant[id].wholeName +'</div>'+
                            '<div class="elementline"></div>'+
                            '</div>');
            }
            appendConsultant(0);
    ...
    function changeChooseProfile(id){
        $.getJSON('data.json',function(data){
            $('.choose-profile .elementchatcontent.speciality').html(data.consultant[id].speciality);
            $('.choose-profile .elementchatcontent.career').html(data.consultant[id].career);
        });
    }
