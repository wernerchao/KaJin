...
if ($_GET['showall']) {
    $sql = $mysql->query('Select * From `message` Order By `timestamp` Asc');
} else {
    $sql = $mysql->query("Select * From `message` Where `replied` = '0' Order By `timestamp` Asc");
}
...
<table>
	...
	<?php
        while ($row = $sql->fetch_assoc()) {
            ...
            echo "<td><a href='101.php?id=".$row['user_id']."'>".$row['user_id'].'</a></td>';
