...
if ($_GET['showall']) {
    $sql = $mysql->query('Select * From `counselor_board` Order By `date` Asc, `time` Asc, `id` Asc');
} else {
    $sql = $mysql->query("Select * From `counselor_board` Where `replied` = '0' Order By `date` Asc, `time` Asc, `id` Asc");
}
