        ...
        if ($date >= get_date()) {
            for ($j = 0; $j <= 23; ++$j) {
                ...
                if ($_POST[$date.'-'.$j] == 1 && !$appointment_data[$date][$j]['id']) {
                    $mysql->query("Insert Into `appointment` (`counselor_id`, `date`, `time`) Values ('$data_counselor[id]', '$date', '$j')");
                }
                // 3. 如果要改成不可預約但現在有則刪除
                if ($_POST[$date.'-'.$j] == 0 && $appointment_data[$date][$j]['id']) {
                    $mysql->query("Delete From `appointment` Where `date` = '$date' And `time` = '$j' And `counselor_id` = '$data_counselor[id]' And `user_id` = '0'");
                }
            }
        }
...
// 讀上週資料
$sql = $mysql->query("Select * From `appointment` Where `date` Between '$start_last' And '$end_last' And `counselor_id` = '$data_counselor[id]'");
while ($row = $sql->fetch_assoc()) {
    $appointment_data[$row['date']][$row['time']] = $row;
}
...
// 印表格
$date = $start;
echo '<form action="401.php" method="post">';
...
echo '　　';
echo "<input type=\"button\" value=\"不修改，返回上一頁\" onclick=\"location.href='400.php'\">";
echo '</center>';
echo '</form>'
